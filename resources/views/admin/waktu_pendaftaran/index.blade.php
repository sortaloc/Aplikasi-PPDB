...
<script>
    Swal.fire({
        icon: 'error',
        title: 'Waktu pendaftaran telah tersedia!'
    })
</script>
@endif
...
<div class="card">
    <div class="card-header">
        <h4>Tabel Jadwal Pendaftaran</h4>
    </div>
    <div class="card-body">
        @if ($waktu == null)
        <div class="float-left">
            <a href="{{ route('adminwaktupendaftaran.create') }}" class="btn btn-icon icon-left btn-primary"><i class="fas fa-plus"></i> Tambah Data</a>
        </div>
        @endif
        ...
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ route('adminwaktupendaftaran.edit', $waktu->id)}}" class="btn btn-icon btn-info"><i class="fas fa-pencil-alt"></i></a>
                        <button class="btn btn-icon btn-danger" onclick="hapusData( <?php echo $waktu->id; ?> )"><i class="fas fa-trash"></i></button>
                        <form id="data-{{ $waktu->id }}" action="{{ route('adminwaktupendaftaran.destroy', $waktu->id)}}" method="post">
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
