...
<div class="card">
    ...
    <div class="card-body">
        ...
        <h6 style="text-align: center;">-- Selamat mengerjakan ! --</h6>
    </div>
</div>
<div class="row">
    @foreach ($mapelujian as $mu)
    <?php
    $nilai = App\Models\Nilai::where('id_user', Auth::user()->id)->get();

    $status_id_mapel = "";
    foreach ($nilai as $n) {
        if ($n->id_mapel == $mu->id_mapel) {
            $status_id_mapel = "ada";
            break;
        } else {
            $status_id_mapel = "tidak_ada";
        }
    }

    if ($status_id_mapel == "tidak_ada") {
    ?>
